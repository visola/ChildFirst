define({
  "button_create_new": "Nova",
  "edit_branch" : "Editando Filial: {{name}}",
  "edit_school" : "Editando Escola: {{name}}",
  "edit_user" : "Editando Usuário: {{email}}",
  "hello" : "Oi {{name}}!",
  "label_branches" : "Filiais",
  "label_schools": "Escolas",
  "label_users": "Usuários",
  "login_with_google" : "Logar com conta do Google",
  "new_branch" : "Nova Filial",
  "new_school" : "Nova Escola",
  "new_user" : "Nova Usuário",
  "please_login" : "Por favor, faça o login.",
  "title_branches" : "Filiais",
  "title_schools" : "Escolas",
  "title_users" : "Usuários"
});
